    ...
    <section class="cto">
        ...
        <div class="cto-overlay">
            <div class="cto-action">
                ...
                <v-btn color="primary" width="200px" height="40px" @click="openEmail()">Inschrijven</v-btn>
            </div>
        </div>
    </section>
...
onMounted(async ()=>{
    if(props.user?.department){
        const res = await API.getAllChallengesForCompany(props.user.department.parentCompany.id);
        companyChallenges.value = res;
    }
    const all = await API.getChallengesBySearch(undefined,undefined,undefined,"newest_first",1);
