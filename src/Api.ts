...
namespace API {
    ...
    export async function departmentExists(companyId : number, name : string){
        const data = await postRequest(`department/exists`, {parentCompanyId: companyId,name:name});
        return data;
    }
    export async function createDepartment(name : string, targetAdminId : number){
        const data = await postRequest(`department/create`, {name:name,adminId:targetAdminId});
        return data;
    }
